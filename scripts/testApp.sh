...
kubectl port-forward --address "$(minikube ip)" svc/inventory-deployment 9443:9444 &
...
curl -k -X POST "https://localhost:9444/dev/api/systems?heapSize=1048576&hostname=localhost&javaVersion=9&osName=linux" | grep "added" || exit 1
curl -k "https://localhost:9444/dev/api/systems" | grep "localhost" || exit 1
