...
Import the [hotspot=metricsImport file=1]`Counted` annotation, and apply the `@Counted` annotation to the [hotspot=metricsAddSystem file=1]`POST /api/systems`, [hotspot=metricsUpdateSystem file=1]`PUT /api/systems/{hostname}`, [hotspot=metricsRemoveSystem file=1]`DELETE /api/systems/{hostname}`, [hotspot=metricsAddSystemClient file=1]`POST /api/systems/client/{hostname}` endpoints
