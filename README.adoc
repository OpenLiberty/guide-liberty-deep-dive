...
Next, you'll investigate how you can externalize Liberty server configuration and inject configuration properties for your microservice by using MicroProfile Config.


=== Enabling configurable ports and context root

So far you've been using hard-coded values to set the HTTP and HTTPS ports, as well as the context root for the Liberty server. Those configuration can be externalized to change these values when you run or deploy your microservice on different environment.
...
You'll now explore how to make use of MicroProfile's Config API to inject static configuration into your microservices.
...
Edit the [hotspot=printClientPort file=0]`POST` request within the `SystemResource` class, so that the `/inventory/api/systems/client/{hostname}` endpoint prints out the `CLIENT_PORT` value.

Revisit the OpenAPI UI `http://localhost:9080/openapi/ui/` to view these changes. Open the `/api/systems/client/{hostname}` endpoint and execute it within the UI. You can view the `CLIENT_PORT` value from the console where the dev mode was started.

=== Adding microprofile-config.properties file

Define the configurable varibles in the `microprofile-config.properties` configuration file for the MicroProfile Config.
...
Revisit the OpenAPI UI `http://localhost:9080/openapi/ui/` to view the `CLIENT_PORT` value from the console.
