...
Configure the client https port by setting the [hotspot=https file=2]`<liberty.var.client.https.port>` to [hotspot=https file=2]`9444`.

You can check that the JWT is set up by making requests to the `/client/{hostname}` endpoint you updated. 
