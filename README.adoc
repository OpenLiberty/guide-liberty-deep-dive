...
```
This command deploys a ConfigMap named `inv-app-root` to your cluster. It has a key called `contextRoot` with a value of `/dev`. The `--from-literal` flag specify individual key-value pairs to store in this ConfigMap. 
