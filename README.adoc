...
The MicroProfile OpenAPI API is included in the [hotspot=mp5 file=0]`microProfile` dependency that is specified in your `pom.xml` file. The [hotspot=mp5 file=1]`microProfile-5.0` feature that includes the `mpOpenAPI` feature is also enabled in the `server.xml` file.
...
Because the Open Liberty server was started in development mode at the beginning of the guide, your changes were automatically picked up. Visit the http://localhost:9080/openapi/ URL to see the updated endpoint descriptions. The endpoints at which your restfulWS methods are served are now more meaningful:
