...
Instead of hard-coded the `user` and `password` values in the [hotspot=postgresUser file=0]`properties.postgresql` properties, use the `${postgres/username}` and `${postgres/password}` variables that are defined by [hotspot=variables file=0]`<variable>` elements.
