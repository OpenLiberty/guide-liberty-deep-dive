...
The [hotspot=postgresqlLibrary file=1]`library` element tells the Liberty server where to find the PostgreSQL library. The [hotspot=dataSource file=1]`dataSource` element points to where the Java Database Connectivity (JDBC) connects, along with some database vendor-specific properties. For more information, see the https://www.openliberty.io/docs/latest/reference/config/dataSource.html[dataSource element documentation^].
