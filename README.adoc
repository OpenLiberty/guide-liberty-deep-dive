...
----
#Replace the `server.xml` file.#
`module-metrics/src/main/liberty/config/server.xml`
----
...
Import the [hotspot=metricsImport file=1]`Counted` annotation, and apply the `@Counted` annotation to the POST [hotspot=metricsAddSystem file=1]`/api/systems`, PUT [hotspot=metricsUpdateSystem file=1]`/api/systems/{hostname}`, DELETE [hotspot=metricsRemoveSystem file=1]`/api/systems/{hostname}`, POST [hotspot=metricsAddSystemClient file=1]`/api/systems/client/{hostname}` endpoints
...
Point your browser to the http://localhost:9080/openapi/ui[^] URL to try out your application and call some of the endpoints that you have annotated. Or simply point your browser to the http://localhost:9080/inventory/api/systems[^] URL.
ifdef::cloud-hosted[]
```bash
curl -s http://localhost:9080/inventory/api/systems | jq
```
