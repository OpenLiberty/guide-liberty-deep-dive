...
This interface declares methods for accessing each of the endpoints that are set up for you in the `system` service.
...
The [hotspot=getSystemClient file=1]`getSystemClient` method builds and returns a new instance of the `SystemClient` class for the given hostname. The [hotspot=addSystemClient file=1]`addSystemClient` method uses this method to create a rest client called [hotspot=customRestClient file=1]`customRestClient` to consume the `system` service. 
...
Configure the https port by setting the [hotspot=https file=5]`<liberty.var.client.https.port>` to `9443`.
