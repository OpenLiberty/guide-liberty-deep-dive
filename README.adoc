...
Define the [hotspot=postgresSetup file=3]`GenericContainer` test container to start up the PostgreSQL docker image, and define the [hotspot=libertySetup file=3]`libertyContainer` test container to start up the `inventory` docker image. Make sure that both containers use the same [hotspot=network hotspot=pNetwork hotspot=lNetwork file=3]`network`. The [hotspot=health file=3]`/health/ready` endpoint can tell the container is ready to start testing.
...
The [hotspot=testRemoveSystem file=3]`testRemoveSystem()` verifies the [hotspot=removeSystemstem file=3]`removeSystem` endpoint.


Create the log4j properites file required by the test container.
...
Add all required [hotspot=testDependenies file=5]`dependency`s with `test` scope, including JUnit5, Testcontainers, Log4J, JBoss RESTEasy client, Glassfish JSON, and Vert.x libraries. Also, add the [hotspot=failsafe file=5]`maven-failsafe-plugin` plugin, so that the integration test can be run by the Maven `verify` goal.

=== Running the tests

You can run the Maven `verify` goal that compiles the java files, start the containers, run the tests, and then stop the containers.
