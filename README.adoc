...
Create a RESTful client interface for the `system` microservice in the `inventory` microservice.
...
The [hotspot=getSystemClient file=0]`getSystemClient()` method builds and returns a new instance of the `SystemClient` class for the given hostname. The [hotspot=addSystemClient file=0]`/client/{hostname}` `POST` endpoint uses this method to create a rest client called [hotspot=customRestClient file=0]`customRestClient` to consume the `system` microservice.  

An JWT instance is injected to the [hotspot=jwt file=0]`jwt` field variable by the `jwtSso` feature. It is used to create the authenication header and passed via a parameter to the endpoints of the [hotspot=customRestClient file=0]`customRestClient()` to get the properties from the `system` microservice. This system is then added to the inventory.
...
The `keyStore` must be the same in both your `system` and `inventory` microservice. As the configured `system` microservice is already provided for you, copy the key store file from your `system` microservice to your `inventory` service.
