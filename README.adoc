...
Try changing the value of these variables in the pom.xml file - change default.http.port to `9081`, change the default.https.port to `9445`, change to default.context.root to `/trial`. Because we've been using dev mode, these changes will be automatically picked up by the server.
