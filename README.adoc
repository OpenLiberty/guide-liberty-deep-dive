...
The [hotspot=path file=2]`@Path` annotation on this class indicates that this resource responds to the `/systems` path in the JAX-RS application. The [hotspot=applicationPath file=2]`@ApplicationPath` annotation in the `RestApplication` class together with the [hotspot=path file=2]`@Path` annotation in the `SystemResource` class indicates that this resource is available at the `/api/systems` path.
