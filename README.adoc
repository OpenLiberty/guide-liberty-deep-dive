...
The [hotspot=postgresql file=2]`postgresql` dependency is needed by JPA to connect the PostgreSQL database. The [hotspot=copyDependencies file=2]`copyDependencies` configuration tells the Liberty Maven plugin to copy the library to the Liberty shared resources directory.
