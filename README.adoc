...
You will use `Minikube` as a single-node Kubernetes cluster that runs locally in a virtual machine. Make sure you have `kubectl` installed. If you need to install `kubectl`, see the https://kubernetes.io/docs/tasks/tools/install-kubectl/#install-kubectl-on-linux[kubectl installation instructions^].
...
Expand-Archive -Path .\inventory.zip -DestinationPath .\start
```
--
