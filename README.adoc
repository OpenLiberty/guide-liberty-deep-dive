...
To use PostgreSQL database, you need to download its library and store to the Liberty shared resources directory. Configure the `pom.xml` configuration file.
...
The CRUD operations are defined in the Inventory. To perform these operations by using JPA, you need an [hotspot=Inventory file=0]`Inventory` class. 
