...
Then select the `Generate Project` button. This will download the starter project as `liberty-deepdive-inventory.zip` file. 

Next, unpackage the `liberty-deepdive-inventory.zip` file on your machine. Move the contents of this unzipped `liberty-deepdive-inventory` directory to within the start directory of this project as following path:

[source, role='no_copy']
----
guide-liberty-deepdive/start/liberty-deepdive-inventory
----
...
pom.xml
[source, xml, linenums, role='code_column hotspot file=0']
----
include::finish/module-getting-started/pom.xml[]
----

Your [hotspot]`pom.xml` file is located in the `start/liberty-deepdive-inventory` directory and is configured to include the [hotspot=libertyMavenPlugin]`liberty-maven-plugin`, which allows you to install applications into Liberty and manage the server instances.
...
Because the Open Liberty server was started in development mode at the beginning of the guide, your changes were automatically picked up. Visit the http://localhost:9080/openapi/ URL to see the updated endpoint descriptions. The endpoints at which your JAX-RS methods are served are now more meaningful:
...
OpenAPI annotations can also be added to POJOs to describe what they represent. Currently, the OpenAPI document doesn't have a very meaningful description of the `SystemData` POJO and hence it's very difficult to tell exactly what this POJO is used for. To describe the`SystemData` POJO in more detail, augment the [hotspot file=0]`src/main/java/io/openliberty/deepdive/rest/model/SystemData.java` file with some OpenAPI annotations.
