...
powershell -command Expand-Archive -Path .\inventory.zip
```
--
...
To avoid the conflict with the default ssl configuration, define your own ssl configuration by setting the [hotspot=ssl file=0]`id` attribute, the [hotspot=sslDefault file=0]`sslDefault` element, and the [hotspot=mpJwt file=0]`sslRef` attribute in the `mpJwt` element to another value.
...
```
cd finish
mvnw.cmd -f system\pom.xml liberty:run
```
--
...
[role='command']
```
mvnw.cmd liberty:devc -DdockerRunOpts="-e POSTGRES_HOSTNAME=172.17.0.2" -DserverStartTimeout=240
```
--
...
[role='command']
```
./mvnw liberty:devc -DdockerRunOpts="-e POSTGRES_HOSTNAME=172.17.0.2" -DserverStartTimeout=240
```
--
endif::[]
...
You must run this section in a Linux environment. If you are running Mac or Windows, skip to the next section.
