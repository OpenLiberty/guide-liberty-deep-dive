...
Navigate to the `start/module-persisting-data` directory to begin.
...
Run the following command to start the system service in dev mode in the `module-persistence-data`:
...
To use PostgreSQL library the dependency must be included in the `pom.xml` file, follow the instructions below to add PostgreSQL as a dependency by replacing the pom file.  
