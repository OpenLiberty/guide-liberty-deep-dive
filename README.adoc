...
Once there, we must now enter the properties needed for the application.
...
Now that you have a basic Liberty application running, the next stepo is to create the additional application and resource classes needed for this application. Within these clases you will make use of Jakarta REST, as well as other MicroProfile and Jakarta APIs.
...
Add OpenAPI [hotspot=SystemDataSchema hotspot=hostnameSchema file=0]`@Schema` annotations to the [hotspot=SystemDataSchema file=1]`SystemData` class and the [hotspot=hostnameSchema file=0]`hostname` variable.
