...
Press CTRL+C in the session to stop the dev mode `mvn liberty:dev` that was started in the previous section. Navigate to your application directory if you are not:
...
Retrieve the PostgreSQL container's IP address by running the following:
...
Build and run the container by running the `devc` goal with the PostgreSQL container IP address from the `start/inventory` directory. Because it is the first time to run container, the command extends the timeout.
...
Also, run the following commands to stop the PostgreSQL database container that was started in the previous section.
...
You can check out the service at the https://localhost:31000/openapi/ui/[https://localhost:31000/openapi/ui/^] URL.  The servers dropdown list should show the `https://localhost:31000/inventory`. You can run the following command to access the inventory microservice:
[role='command']
```
...
In the [hotspot file=0]`server.xml`, the [hotspot=contextRoot file=0]`context.root` variable is defined. The context root for the inventory service can be changed using this variable. The value for the [hotspot=contextRoot file=0]`context.root` variable can be defined in a ConfigMap and accessed as an environment variable.
...
You can now check out the service at the https://localhost:30000/openapi/ui/[https://localhost:30000/openapi/ui/^] URL. The servers dropdown list should show the `https://localhost:30000/dev`. You can run the following command to access the inventory microservice:
[role='command']
```
...
Once you are done, you can delete all Kubernetes resources by running the `kubectl delete` commands:
