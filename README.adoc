...
Navigate to your applicaion directory

```
cd start/inventory
```

The first thing you need to do is add in some basic authentication to your runtime so a user can authenicate with the `/metrics` endpoint. 

To do that, open the runtime configuration file server.xml.
...
The [hotspot=quickStartSec file=0]`quickStartSecurity` tag will enable security for the `/metrics` endpoint and allow you to log in with the credentials defined.

// File 1
SystemResource.java
[source, java, linenums, role='code_column']
----
include::finish/module-metrics/src/main/java/io/openliberty/deepdive/rest/resources/SystemResource.java[]
----

[role="code_command hotspot file=1", subs="quotes"]
----
#Replace the `SystemResource` class.#
`src/main/java/io/openliberty/deepdive/rest/resources/SystemResource.java`
----

Apply the [hotspot=MetricsAddNew hotspot=MetricsAddNew2 hotspot=MetricsAddNew3 hotspot=MetricsAddNew4 hotspot=MetricsAddNew5 file=1]`@Counted` annotation to each of the endpoints to count the invocations of the annotated object in this case the calls to our REST API.

Additional information about the annotations MicroProfile metrics provide, relevant metadata fields, and more are available at
...
Now try accessing the endpoints to add to the application metrics.
...
* https://localhost:9443/metrics/base
* https://localhost:9443/metrics/vendor

The `/metrics` endpoint provides you with all the metrics in text format. 

The `/metrics/application` endpoint provides you with applicaion specific metrics.

The `/metrics/base` endpoint provides you with metrics that are defined in MicroProfile specifications.
...
Firstly, point your browser to the http://localhost:9443/metrics MicroProfile Metrics endpoint. Log in as the admin user with adminpwd as the password. You can see both the system and application metrics in a text format.

To see only the application metrics, point your browser to https://localhost:9443/metrics/application.

== Building and running the application

The Open Liberty server was started in development mode at the beginning of the deepdive and all the changes were automatically picked up.

// static guide instructions:
ifndef::cloud-hosted[]
Point your browser to the https://localhost:9443/metrics[https://localhost:9443/metrics^] URL to review all the metrics 
that are enabled through MicroProfile Metrics. Log in with `admin` as your username and `adminpwd` as your password. 
