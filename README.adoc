...
First, make a POST request to the `/inventory/api/systems/` endpoint. To make this request, expand the POST
...
The POST request adds a user-specified heapsize, hostname, javaVersion, and an osName to the inventory list of systems.

Next, make a GET request to the `/inventory/api/systems` endpoint. To make this request, expand the GET
endpoint on the UI, click the `Try it out` button,
and then click the `Execute` button. The GET request
returns an ID, and all the items from your system.
