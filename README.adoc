...
First, make a POST request to the `/api/systems/` endpoint. To make this request, expand the first POST endpoint on the UI, click the `Try it out` button, provide `heapSize`, `hostname`, `javaVersion`, and `osName`, and then click the `Execute` button. The POST request adds a system with the specified values to the database.
...
Next, make a PUT request to the `/api/systems/{hostname}`. To make this request, expand the PUT endpoint on the IO, click the `Try it out` button, provide a heapSize, hostname, javaVersion, and an osName, and then click the `Execute` button. The PUT request updates the system with the specified values. Run the GET request again to see the updated properties.

Next, make a DELETE request to the `api/systems/{hostname}`. To make this request, expand the DELETE endpoint on the IO, click the `Try it out` button, and then click `Execute`. The DELETE request removes the system from the database. 
