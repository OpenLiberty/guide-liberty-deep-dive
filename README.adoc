...
The microservice that you’ll be working with is called `inventory`. The `inventory` microservice persists data into a PostgreSQL database. You will also learn how to secure the REST endpoints and use JSON web token to communicate another provided `system` microservice.
