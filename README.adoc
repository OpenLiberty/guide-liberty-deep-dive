...
First, make a POST request to the `/api/systems/` endpoint. To make this request, expand the first POST endpoint on the UI, click the `Try it out` button, provide values to the `heapSize`, `hostname`, `javaVersion`, and `osName` fields, and then click the `Execute` button. The POST request adds a system with the specified values to the database.
...
Next, make a PUT request to the `/api/systems/{hostname}`. To make this request, expand the PUT endpoint on the IO, click the `Try it out` button, provide different values to the `heapSize`, `javaVersion`, and `osName` fields, and then click the `Execute` button. The PUT request updates the system with the specified values. 

To see the updated system, make a GET request to the `/api/systems/{hostname}` endpoint. To make this request, expand the GET endpoint on the UI, click the `Try it out` button, and then click the `Execute` button. The GET request returns the system from the database.
