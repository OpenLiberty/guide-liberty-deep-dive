...
Instead of hard-coded the `user` and `password` values in the [hotspot=postgresUser file=0]`properties.postgresql` properties, use `${postgres/username}` and `${postgres/password}` that are defined by the [hotspot=variables file=0]`postgres/username` and [hotspot=variables file=0]`postgres/password` variable elements.
