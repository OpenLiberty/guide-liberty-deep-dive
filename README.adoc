...
When you're finished trying out the microservice, press `CTRL+C` in the command-line session whewre you started dev mode to stop and remove the container.
...
Create the log4j properites file that are required by the Testcontainers framework.
...
You can use WebSphere Liberty for development even if you haven't purchased it, but if you have production entitlement you can easily change to use it, with the following steps
