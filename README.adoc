...
Add OpenAPI [hotspot=addSystemAPIResponse hotspot=updateSystemAPIResponse hotspot=removeSystemAPIResponse hotspot=addSystemClientAPIResponses file=1]`@APIResponse`, [hotspot=addSystemAPIResponses hotspot=updateSystemAPIResponses hotspot=removeSystemAPIResponses hotspot=addSystemClientAPIResponse file=1]`@APIResponses`, [hotspot=listContentsAPIResponseSchema hotspot=getSystemAPIResponseSchema file=1]`@APIResponseSchema`, [hotspot=listContentsOperation hotpost=getSystemOperation hotspot=addSystemOperation hotspot=updateSystemOperation hotspot=removeSystemOperation hotspot=addSystemClientOperation file=1]`@Operation`, [hotspot=addSystemParameters hotspot=updateSystemParameters file=1]`@Parameters`, and [hotspot=getSystemParameter hotspot=addSystemParameter hotspot=updateSystemParameter hotspot=removeSystemParameter hotspot=addSystemClientParameter file=1]`@Parameter` annotations to the JAX-RS methods, [hotspot=listContents file=1]`listContents()`, [hotspot=getSystem file=1] `getSystem`, [hotspot=addSystem file=1] `addSystem`, [hotspot=updateSystem file=1] `updateSystem`, [hotspot=removeSystem file=1] `removeSystem`, and [hotspot=addSystemClient file=1] `addSystemClient`.
...
| [hotspot=addSystemAPIResponse hotspot=updateSystemAPIResponse hotspot=removeSystemAPIResponse hotspot=addSystemClientAPIResponses file=1]`@APIResponse`  | Describes a single response from an API operation.
| [hotspot=addSystemAPIResponses hotspot=updateSystemAPIResponses hotspot=removeSystemAPIResponses hotspot=addSystemClientAPIResponse file=1]`@APIResponses`  | Describes multiple responses from an API operation.
| [hotspot=listContentsAPIResponseSchema hotspot=getSystemAPIResponseSchema file=1]`@APIResponseSchema` | Convenient short-hand way to specify a simple response with a Java class that could otherwise be specified using @APIResponse.
| [hotspot=listContentsOperation hotpost=getSystemOperation hotspot=addSystemOperation hotspot=updateSystemOperation hotspot=removeSystemOperation hotspot=addSystemClientOperation file=1]`@Operation`    | Describes a single API operation on a path.
| [hotspot=addSystemParameters hotspot=updateSystemParameters file=1]`@Parameters`    | Describes multiple operation parameters.
| [hotspot=getSystemParameter hotspot=addSystemParameter hotspot=updateSystemParameter hotspot=removeSystemParameter hotspot=addSystemClientParameter file=1]`@Parameter`    | Describes a single operation parameter.
|===
...
To describe the`SystemData` POJO in more detail, augment the [hotspot file=2]`src/main/java/io/openliberty/deepdive/rest/model/SystemData.java` file with some OpenAPI annotations.
...
Add OpenAPI [hotspot=SystemDataSchema hotspot=hostnameSchema file=2]`@Schema` annotations to the [hotspot=SystemDataSchema file=2]`SystemData` class and the [hotspot=hostnameSchema file=2]`hostname` variable.
...
Add variables for the [hotspot=portVariables file=0]HTTP port, [hotspot=portVariables file=0]HTTPS port and for the [hotspot=contextRootVariable file=0]context root to the server.xml. Change the [hotspot=editedHttpEndpoint file=0]httpEndpoint to reflect the new HTTP port variables and change the [hotspot=editedContextRoot file=0]contextRoot to utilise this new variable too.

Add configurtaion for the [hotspot=PortConfiguration file=1]HTTP port, [hotspot=PortConfiguration file=1]HTTPS port and the [hotspot=ContextRootConfiguration file=1]context root to the pom.xml file. 


To edit this configuration, we will re-run the application using dev mode with the addition of new values for these configuration variables.
...
The [hotspot=injectConfig file=2]@Inject annotation injects the client HTTPS port number directly, the injection value is static and fixed on application starting.

Edit the [hotspot=printClientPort file=2]POST request within the `SystemResource` class, /inventory/api/systems/client/{hostname}, to print out the CLIENT_PORT value.
...
----
microprofile-onfig.properties
[source, Java, linenums, role='code_column hide_tags=copyright']
----
include::finish/module-config/src/main/resources/META-INF/microprofile-config.properties[]
----

The [hotspot=ordinal file=3]`config_ordinal` variable in this properties file enables us to set the ordinal of this file and thus what other ConfigSources it could overwrite.

The [hotspot=configPort file=3]`io_openliberty_inventory_ClientPortOverwrite` variable enables the Client Port to be overwritten.

Visit the following http://localhost:9080/inventory/api/systems/client/{hostname} to view this edited CLIENT_PORT value.


We'll now stop and restart the application to revert back to the default configuration for the remainder of the modules.
