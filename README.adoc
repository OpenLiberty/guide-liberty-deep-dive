...
Build and run the container by running the `devc` goal from the `start/inventory` directory. Because it is the first time to run container, the command extends the timeout.

[role='command']
```
mvn liberty:devc -DserverStartTimeout=120
```
