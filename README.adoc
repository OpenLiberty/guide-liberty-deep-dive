...
Check out the service that you created at the http://localhost:9090/inventory/api/systems URL. If successful, it returns `[]` to you.
...
```bash
curl 'http://localhost:9090/inventory/api/systems'
```
...
Because the Liberty was started in dev mode at the beginning of this exercise, your changes were automatically picked up. Go to the http://localhost:9090/openapi URL to see the updated endpoint descriptions. The endpoints at which your REST methods are served now more meaningful:

ifdef::cloud-hosted[]
```bash
curl http://localhost:9090/openapi
```
endif::[]
...
- url: https://localhost:9453/inventory
paths:
  /api/systems:
    get:
...
You can also visit the http://localhost:9090/openapi/ui URL to see each endpoint's updated description. Click each of the icons within the UI to see the updated descriptions for each of the endpoints.
ifdef::cloud-hosted[]
In this Skills Network environment, simply click the following button:

::startApplication{port="9090" display="external" name="Visit OpenAPI UI" route="/openapi/ui"}
endif::[]
...
Refresh the http://localhost:9090/openapi URL to see the updated OpenAPI tree. You should see much more meaningful data for the Schema:
...
Again, you can also view this at the http://localhost:9090/openapi/ui URL. Scroll down in the UI to the schemas section and open up the SystemData schema icon.
...
* [hotspot=httpPort file=1]`liberty.var.default.http.port` to `9091`
* [hotspot=httpsPort file=1]`liberty.var.default.https.port` to `9454`
* [hotspot=contextRoot file=1]`liberty.var.default.context.root` to `/trial`.

Because you are using dev mode, these changes are automatically picked up by the Liberty instance.

ifndef::cloud-hosted[]
Now, you can access the application by the http://localhost:9091/trial/api/systems URL. Alternatively, for the updated OpenAPI UI, use the following URL http://localhost:9091/openapi/ui/.
endif::[]

ifdef::cloud-hosted[]
Now, you can access the application by running the following command:
```bash
curl http://localhost:9091/trial/api/systems
```

Alternatively, for the updated OpenAPI UI, click the following button to visit ***/openapi/ui*** endpoint:

::startApplication{port="9091" display="external" name="Visit OpenAPI UI" route="/openapi/ui"}
endif::[]

When you are finished trying out changing this configuration, change the variables back to their original values.

* update [hotspot=httpPort file=1]`liberty.var.default.http.port` to `9090`
* update [hotspot=httpsPort file=1]`liberty.var.default.https.port` to `9453`
...
Revisit the OpenAPI UI http://localhost:9090/openapi/ui to view these changes. Open the `/api/systems/client/{hostname}` endpoint and run it within the UI to view the `CLIENT_PORT` value.
...
```bash
curl -s 'http://localhost:9090/inventory/api/systems' | jq
```
...
```bash
curl -X PUT 'http://localhost:9090/inventory/api/systems/localhost?heapSize=2097152&javaVersion=11&osName=linux'
```
...
```bash
curl -s 'http://localhost:9090/inventory/api/systems/localhost' | jq
```

Next, make a DELETE request to the ***/api/systems/{hostname}*** endpoint. The DELETE request removes the system from the database.

```bash
curl -X DELETE 'http://localhost:9090/inventory/api/systems/localhost'
```
...
[role='command']
----
curl -X POST 'http://localhost:9090/inventory/api/systems?hostname=localhost&osName=mac&javaVersion=11&heapSize=1'
----
...
[role='command']
----
curl -k --user alice:alicepwd -X DELETE 'https://localhost:9453/inventory/api/systems/localhost'
----
...
Configure the client https port by setting the [hotspot=https file=2]`<liberty.var.client.https.port>` to `9454`.
...
You can check that the `system` microservice is secured against unauthenticated requests at the https://localhost:9454/system/api/heapsize URL. You can expect to see the following error in the console of the `system` microservice:

ifdef::cloud-hosted[]
Open another command-line session and run the following command:
```bash
curl -k 'https://localhost:9454/system/api/heapsize'
```
endif::[]
...
[role='command']
----
curl -k --user bob:bobpwd -X POST 'https://localhost:9453/inventory/api/systems/client/localhost'
----
...
The [hotspot=Startup file=0]`@Startup` annotation indicates that this class is a startup health check procedure. Navigate to the http://localhost:9090/health/started URL to check the status of the startup health check. In this case, you are checking the cpu usage. If more than 95% of the cpu is being used, a status of `DOWN` is returned.
ifdef::cloud-hosted[]
```bash
curl -s http://localhost:9090/health/started | jq
```
endif::[]
...
----
The [hotspot=Readiness file=2]`@Readiness` annotation indicates that this class is a readiness health check procedure. Navigate to the http://localhost:9090/health/ready URL to check the status of the readiness health check. This readiness check tests the connection to the PostgreSQL container that was created earlier in the guide. If the connection is refused, a status of `DOWN` is returned.
...
Or, you can visit the http://localhost:9090/health URL to see the overall health status of the application.
...
ifndef::cloud-hosted[]
Point your browser to the http://localhost:9090/openapi/ui[^] URL to try out your application and call some of the endpoints that you annotated.
endif::[]
...
curl -k --user bob:bobpwd -X DELETE \
  'https://localhost:9453/inventory/api/systems/localhost'
```
...
curl -k --user alice:alicepwd -X PUT \
  'http://localhost:9090/inventory/api/systems/localhost?heapSize=2097152&javaVersion=11&osName=linux'
```
...
ifndef::cloud-hosted[]
Point your browser to the https://localhost:9453/metrics[^] URL to review all the metrics that are enabled through MicroProfile Metrics. Log in with `bob` as your username and `bobpwd` as your password. You can see the metrics in text format.

To see only the application metrics, point your browser to https://localhost:9453/metrics?scope=application[^]. You can expect to see your application metrics in the output.
...
You can see the system metrics at the https://localhost:9453/metrics?scope=base[^] URL. You can also see the vendor metrics at the https://localhost:9453/metrics?scope=vendor[^] URL.
endif::[]
...
```bash
curl -k --user bob:bobpwd https://localhost:9453/metrics?scope=application
```
...
```bash
curl -k --user bob:bobpwd https://localhost:9453/metrics\?scope=base
```

To see the vendor metrics, run the following curl command:
```bash
curl -k --user bob:bobpwd https://localhost:9453/metrics\?scope=vendor
```

To review all the metrics, run the following curl command:
```bash
curl -k --user bob:bobpwd https://localhost:9453/metrics
```
endif::[]
...
ee2daf0b33e1  inventory-dev-mode  "/opt/ol/helpers/run…"  2 minutes ago  Up 2 minutes  0.0.0.0:7777->7777/tcp, 0.0.0.0:9090->9090/tcp, 0.0.0.0:9453->9453/tcp  liberty-dev
----

ifndef::cloud-hosted[]
Point your browser to the http://localhost:9090/openapi/ui URL to try out your application. 
endif::[]
...
[role='command']
```
curl https://raw.githubusercontent.com/OpenLiberty/open-liberty-operator/main/deploy/releases/1.2.1/kubectl/openliberty-app-rbac-watch-all.yaml -o openliberty-app-rbac-watch-all.yaml
...
[role='command']
```
curl https://raw.githubusercontent.com/OpenLiberty/open-liberty-operator/main/deploy/releases/1.2.1/kubectl/openliberty-app-operator.yaml -o openliberty-app-operator.yaml
...
ifndef::cloud-hosted[]
The application might take some time to get ready. See the https://localhost:9453/health URL to confirm that the `inventory` microservice is up and running.

Once your application is up and running, you can check out the service at the https://localhost:9453/openapi/ui/[https://localhost:9453/openapi/ui/^] URL. The servers dropdown list shows the `\https://localhost:9453/inventory` URL. Or, you can run the following command to access the inventory microservice:
[role='command']
```
curl -k https://localhost:9453/inventory/api/systems
```
endif::[]
...
```bash
curl -k https://localhost:9453/health | jq
```
...
```bash
curl -k -X POST 'https://localhost:9453/inventory/api/systems?heapSize=1048576&hostname=localhost&javaVersion=9&osName=linux'
```

```bash
curl -k --user alice:alicepwd -X PUT \
  'https://localhost:9453/inventory/api/systems/localhost?heapSize=2097152&javaVersion=11&osName=linux'
```

```bash
curl -k -s 'https://localhost:9453/inventory/api/systems' | jq
```
endif::[]
...
The [hotspot=contextRoot file=0]`default.context.root` variable is defined in the [hotspot file=0]`server.xml` configuration file. The context root for the inventory service can be changed by using this variable. The value for the [hotspot=contextRoot file=0]`default.context.root` variable can be defined in a ConfigMap and accessed as an environment variable.
...
ifndef::cloud-hosted[]
The application might take some time to get ready. See the https://localhost:9453/health URL to confirm that the `inventory` microservice is up and running. You can now check out the service at the https://localhost:9453/openapi/ui/[https://localhost:9453/openapi/ui/^] URL. The servers dropdown list shows the `\https://localhost:9453/dev` URL. Or, you can run the following command to access the inventory microservice:
[role='command']
```
curl -k https://localhost:9453/dev/api/systems
```
endif::[]
...
```bash
curl -k --user bob:bobpwd -X DELETE \
  'https://localhost:9453/dev/api/systems/localhost'
```

```bash
curl -k -X POST 'https://localhost:9453/dev/api/systems?heapSize=1048576&hostname=localhost&javaVersion=9&osName=linux'
```

```bash
curl -k --user alice:alicepwd -X PUT \
  'https://localhost:9453/dev/api/systems/localhost?heapSize=2097152&javaVersion=11&osName=linux'
```

```bash
curl -k -s 'https://localhost:9453/dev/api/systems' | jq
```
endif::[]
